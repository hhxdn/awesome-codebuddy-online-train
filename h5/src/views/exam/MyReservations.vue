<template>
  <div class="my-reservations-page page-fade-in">
    <van-nav-bar title="我的预约" left-text="返回" left-arrow @click-left="$router.back()" :border="false" />
        ...
        <div v-for="item in list" :key="item.id" class="reservation-card">
          ...
          <div class="card-body">
            ...
            <div class="info-row">
              ...
              <span>满分{{ item.totalScore }}分 / 及格{{ item.passScore }}分</span>
            </div>
...
async function handleCancel(item) {
  try {
    ...
    await put(`/exam/reservations/${item.id}/cancel`)
    showToast('预约已取消')
