<template>
  <div class="enroll-page">
    <!-- 步骤1：报考条件 -->
    <div v-if="step === 1" class="step-conditions">
      <div class="page-header">
        <van-icon name="arrow-left" size="20" @click="handleBack" class="back-btn" />
        <h2>报考条件</h2>
        <p class="subtitle">请仔细阅读以下报考条件</p>
      </div>
      <div class="conditions-card">
        <div class="conditions-content">{{ conditions }}</div>
      </div>
      <div class="step-action">
        <van-button round block type="primary" size="large" @click="step = 2" class="action-btn">
          已阅，开始填写信息
        </van-button>
      </div>
    </div>
    ...
    <div v-else class="step-form">
      <div class="page-header">
        <van-icon name="arrow-left" size="20" @click="step = 1" class="back-btn" />
        <h2>报名学习</h2>
        <p class="subtitle">请填写以下信息完成报名</p>
      </div>
...
async function fetchConditions() {
  try {
    const res = await get('/config/enrollment-conditions')
    if (res.data?.content) conditions.value = res.data.content
  } catch (e) {
    conditions.value = '1. 遵纪守法，品行端正\n2. 具有大专及以上学历\n3. 身体健康，无不良嗜好\n4. 具备相关工作经验优先'
  }
}
