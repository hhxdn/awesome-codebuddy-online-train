    ...
    <div class="card-container">
      <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px;">
        ...
        <div style="display: flex; gap: 10px;">
          <el-select v-model="filterExamId" placeholder="选择线下考试" clearable filterable @change="fetchData" style="width: 220px;">
      ...
      <el-table :data="tableData" border stripe>
        ...
        <el-table-column prop="examTitle" label="考试名称" min-width="180" show-overflow-tooltip />
...
async function fetchData() {
  ...
  try {
    ...
    if (filterExamId.value) params.examPaperId = filterExamId.value
    if (filterStatus.value) params.status = filterStatus.value
    const res = await get('/admin/reservations', params)
...
async function handleConfirm(row) {
  try {
    await ElMessageBox.confirm('确认该学员的预约吗？', '确认预约', { type: 'info' })
    await put(`/admin/reservations/${row.id}/confirm`)
...
async function handleCancel(row) {
  try {
    ...
    await put(`/admin/reservations/${row.id}/cancel`)
...
async function handleComplete(row) {
  try {
    await ElMessageBox.confirm('标记该考试为已完成？', '标记完成', { type: 'info' })
    await put(`/admin/reservations/${row.id}/complete`)
